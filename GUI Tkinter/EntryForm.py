...
class EntryForm(ttk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # form headers
        ttk.Label(self, text='Please enter your contact information',
                  width=60).grid(columnspan=3, pady=10)

        # create label/entry rows
        for i, label in enumerate(['name', 'address', 'phone']):
            ttk.Label(self, text=label.title()).grid(
                row=i + 1, column=0, sticky='ew', pady=10, padx=(0, 10))
            ttk.Entry(self, textvariable=label).grid(
                row=i + 1, column=1, columnspan=2, sticky='ew')

        # submit button
        self.submit = ttk.Button(
            self, text='Submit', style='success.TButton', command=self.print_form_data)
        self.submit.grid(row=4, column=0, sticky='ew', pady=10, padx=(0, 10))

        # cancel button
        self.cancel = ttk.Button(
            self, text='Cancel', style='danger.TButton', command=self.quit)
    ...
    def print_form_data(self):
        ...
        self.new.title('Form Summited')
        self.new.geometry('200x100')
        ttk.Label(self.new, text='Name: {}'.format(self.name.get())).grid(
            row=0, column=0, sticky='ew', padx=(0, 10))
        ttk.Label(self.new, text='Address: {}'.format(self.address.get())).grid(
            row=1, column=0, sticky='ew', padx=(0, 10))
        ttk.Label(self.new, text='Phone: {}'.format(self.phone.get())).grid(
            row=2, column=0, sticky='ew', padx=(0, 10))
        ttk.Button(self.new, text='Close', command=self.new.destroy).grid(
