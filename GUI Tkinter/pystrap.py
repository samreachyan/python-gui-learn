...
class Calculator(ttk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.display = ttk.Label(
            self, textvariable=self.display_var, font='TkFixedFont 20', anchor='e')
        self.display.grid(row=0, column=0, columnspan=4,
                          sticky='ew', pady=15, padx=10)
        ...
        for i, row in enumerate(button_matrix):
            for j, lbl in enumerate(row):
                ...
                btn.grid(row=i + 1, column=j, sticky='nsew',
                         padx=1, pady=1, ipadx=10, ipady=10)
    ...
    def press_button(self, event):
        ...
        if isinstance(value, int):
            if self.position_is_left:
                ...
                self.position_right = str(
                    value) if self.position_right == '0' else f'{self.position_right}{value}'
        elif value == '.':
        ...
        self.display_var.set(
            '.'.join([self.position_left, self.position_right]))
